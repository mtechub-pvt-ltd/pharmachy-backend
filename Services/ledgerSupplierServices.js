const ledgerModel = require("../models/ledgerSupplierModel");
const mongoose = require("mongoose");
const invoiceModel = require("../models/invoiceSupplierModel");
...
exports.getAllledgers = (req, res) => {
    ...
    }).sort({ $natural: -1 }).populate('supplier_id')
}
// // Get ledger 
exports.getSpecificledger = (req, res) => {

    const supplier_id = req.body.supplier_id;
    ledgerModel.find({ supplier_id: supplier_id }, function (err, foundResult) {
        try {
    ...
    }).populate('supplier_id').populate('invoices')
}
...
exports.createledger = async (req, res) => {
    ...
    ledgerModel.find({ supplier_id: req.body.supplier_id }, (error, result) => {
        if (error) {
            ...
            if (result === undefined || result.length == 0) {
                ...
                invoiceModel.find({ supplier_id: req.body.supplier_id }, function (err, foundResult) {
                    try {
                        let overdueAmount = 0;
                        let Partial_Paid = 0;
                ...
                invoiceModel.find({ supplier_id: req.body.supplier_id, $or: [{ invoice_status: 'partially paid' }, { invoice_status: 'no paid' }, { invoice_status: 'overdue' }] }, function (err, foundResult) {
                    try {
                        let overdueAmount = 0;
                        let Partial_Paid = 0;
                        let Total_Amount_Payable_Customer = 0
